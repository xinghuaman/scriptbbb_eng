...
ramdisk(){
  ...
  echo -e " \e[38;5;154m----------\e[0m"
  echo -e " [3]  Ramdisk mode 1 - Squeezelite, Squeezelite-R2, Mpd, Networkaudiod, Upmpdcli"
  echo -e " [4]  Ramdisk mode 2 - Squeezelite, Squeezelite-R2, Mpd, Networkaudiod, RoonBridge, Upmpdcli"
  echo -e " [5]  Ramdisk mode 3 - Squeezelite, Squeezelite-R2, Mpd, Networkaudiod, Upmpdcli (B)"
  echo -e " [6]  Ramdisk mode 4 - Squeezelite, Squeezelite-R2, Mpd, Networkaudiod, RoonBridge, Upmpdcli (B)"
  echo -e " \e[38;5;154m----------\e[0m"
...
echo -e "$Yellow press a key to continue $Color_Off" ; read -n1 ; ramdisk ;;
3)
cp /gentooplayer/script/ramdisk1.sh /etc/default/ramdisk.sh ;
echo -e "$Yellow do you want to start the ramdisk? y/n $Color_Off"
read want
	if [ "$want" = "y" ]; then
    /etc/default/ramdisk.sh
    fi ;
echo
echo -e "$Yellow press a key to continue $Color_Off" ; read -n1 ; ramdisk ;;
4)
cp /gentooplayer/script/ramdisk2.sh /etc/default/ramdisk.sh ;
echo -e "$Yellow do you want to start the ramdisk? y/n $Color_Off"
read want
	if [ "$want" = "y" ]; then
    /etc/default/ramdisk.sh
...
echo -e "$Yellow press a key to continue $Color_Off" ; read -n1 ; ramdisk ;;
5)
cp /gentooplayer/script/ramdisk3.sh /etc/default/ramdisk.sh ;
echo -e "$Yellow do you want to start the ramdisk? y/n $Color_Off"
read want
	if [ "$want" = "y" ]; then
    /etc/default/ramdisk.sh
...
echo -e "$Yellow press a key to continue $Color_Off" ; read -n1 ; ramdisk ;;
6)
cp /gentooplayer/script/ramdisk4.sh /etc/default/ramdisk.sh ;
echo -e "$Yellow do you want to start the ramdisk? y/n $Color_Off"
read want
	if [ "$want" = "y" ]; then
    /etc/default/ramdisk.sh
