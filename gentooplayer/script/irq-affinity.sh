...
#cho -e "\e[1;38;5;88m$(cat /gentooplayer/script/irq-affinity-start.sh | cut -b20-21)\e[0m"
echo -e "\e[1;38;5;88m$(cat /gentooplayer/script/irq-affinity-start.sh | awk '{print $4}' | sed 's/\///g'|sed 's/\procirq//g'|sed 's/\_//g' | sed 's/smpaffinity//g')\e[0m"
echo -e "cancel the current settings? y/n"
read cancellare
  if [ "$cancellare" = "y" ]; then
    echo "#!/bin/bash" > /gentooplayer/script/irq-affinity-start.sh
    fi
echo -e "IRQ already set"
#echo -e "\e[1;38;5;88m$(cat /gentooplayer/script/irq-affinity-start.sh | cut -b20-21)\e[0m"
echo -e "\e[1;38;5;88m$(cat /gentooplayer/script/irq-affinity-start.sh | awk '{print $4}' | sed 's/\///g'|sed 's/\procirq//g'|sed 's/\_//g' | sed 's/smpaffinity//g')\e[0m"
echo -e " \e[38;5;154m────────────────────────────────────────────\e[0m"
...
echo "echo "$cpu" > /proc/irq/$irq/smp_affinity" >> /gentooplayer/script/irq-affinity-start.sh
