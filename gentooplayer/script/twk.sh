...
function select_twk() {
  ...
  5) sisable linux console
  
  6) sisable SSH server
...
echo -e "Some info on the options that will follow are in the file /gentooplayer/script/twk_info"
echo -e "do they want to view now? y/n"
read vogliono
 if [ "$vogliono" = "y" ]; then
      less /gentooplayer/script/twk_info
    fi  
